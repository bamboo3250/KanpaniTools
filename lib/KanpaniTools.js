(function(){
    ...
    window.Kanpani = {
        ...
        HOST: 'http://67.205.150.236',  // staging
        ...
        notifyUserNewVersionIfNeeded: function() {
            ...
            this.checkNewVersion(function(isNewVersion) {
                ...
                var popupTitle = KTUIManager.getTranslatedText('POPUP_NEW_VERSION_TITLE');
                var popupContent = KTUIManager.getTranslatedText('POPUP_NEW_VERSION_CONTENT');
        ...
        init: function(isPanel = true, callback) {
            ...
            KTGameDataManager.loadFromLocalStorage(function() {
                ...
                KTConfigManager.loadFromLocalStorage(function() {
                    ...
                    KTUIManager.changeLanguage('en', function() {
                        KTUIManager.changeLanguage(KTConfigManager.config['language'], function() {
        ...
        readQuestLog: function(body) {
            ...
            for(var i=0;i<commands.length;i++) {
                ...
                if (command['command_type'] == 'page') {
                    ...
                    if (charaId == 'ceo_') {
                        name = KTUIManager.getTranslatedText('DIALOGUES_CEO_NAME');
                    } else if (charaId == 'fairy_') {
                        name = KTUIManager.getTranslatedText('DIALOGUES_FAIRY_NAME');
                    } else {
                        name = command['card_name'];
                        var translatedName = KTCharacterManager.getEmployeeName(charaId);
                        if (KTUIManager.lang != 'jp' && translatedName) name = translatedName;
                    }
        ...
        updateResourceTimer: function(type) {
            ...
            if (last == null) return;
            if (rate == 0) {
                $('#timer-' + type).text(KTUIManager.getTranslatedText('RESOURCE_CHECK_FACILITY'));
                return;
            }
            ...
            if (now.valueOf() < lastDate.valueOf()) {
                ...
                KTUIManager.setResourceTimer(type, KTUIManager.getTranslatedText('RESOURCE_FULL'));
            }
