(function(){
    ...
    function getTranslatedBuffType(buffName) {
        ...
        var translatedBuffType = KTTranslationManager.getUITranslatedText('EMPLOYEES_SKILL_BUFF_TYPE_' + buffName.toUpperCase());
    ...
    window.KTUIManager = {
        ...
        updateDungeonDefcon: function() {
            $('#dungeon-content').empty();

            if (!KTPlayerManager.defconEndTime) return;

            var endTime = new Date(KTPlayerManager.defconEndTime);
            var now = new Date();
            if (now.valueOf() < endTime.valueOf()) {
                var remainTime = new KGTime(endTime.valueOf() - now.valueOf());

                $('#dungeon-content').append(
                    '<div class="schedule-item dungeon-item">&nbsp;' + KTTranslationManager.getUITranslatedText('SCHEDULE_DUNGEON_CAUTIOUS_ENEMY')  + ' <span style="float:right">' + remainTime + '&nbsp;</span></div>'
                );
            }
        },
