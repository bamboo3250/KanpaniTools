(function(){
    ...
    function updateEmployeePanel(employeePanel, employee, callback) {
        ...
        if (player.getKToken()) {
            var kUsername = player.getKUsername();
        ...
        if (nextExp > 0) {
            employeePanel.find('.employee-current-exp').width((currentExp-baseExp)/(nextExp-baseExp)*120);
            employeePanel.find('.employee-remain-exp-text').text(nextText + ': ' + (nextExp-currentExp).toLocaleString('en-US') + ' EXP');
            employeePanel.find('.employee-remain-exp').width((currentExp-baseExp)/(nextExp-baseExp)*120);    
        } else {
            employeePanel.find('.employee-current-exp').width(120);
            employeePanel.find('.employee-remain-exp-text').text(nextText + ': 0 EXP');
            employeePanel.find('.employee-remain-exp').width(120);    
        }
        ...
        if (promo > 0) {
            ...
            employeePanel.find('.promo-thumbnail').show();
        } else {
            employeePanel.find('.promo-thumbnail').hide();
        }
        ...
        const STATS_KEYS = [
            'attack', 'defence', 'mattack', 'mdefence', 'max_hp',
            'accuracy', 'critical', 'evasion',
            'str', 'vit', 'inte', 'pie', 'agi', 'dex', 'luk'
        ];
        var expanderStatsDict = {};
        for(var j=0;j<employee.equipments.length;j++) {
            var equip = employee.equipments[j];

            for(var k=0;equip.expanders && k<equip.expanders.length;k++) {
                var expander = equip.expanders[k];

                for(var l=0;l<STATS_KEYS.length;l++) {
                    var key = STATS_KEYS[l];
                    var name = KTTranslationManager.getUITranslatedText(BONUS_NAME_MAPPING[key]) || key;
                    var value = parseInt(expander[key]);
                    if (value > 0) {
                        if (typeof expanderStatsDict[name] == 'undefined') expanderStatsDict[name] = 0;
                        expanderStatsDict[name] += value;
                    }
                }
                for(var l=0;expander.specials && l<expander.specials.length;l++) {
                    var key = expander.specials[l].param_type_name;    //display_status , param_type_name
                    var name = KTTranslationManager.getUITranslatedText('EMPLOYEES_RUNE_BUFF_TYPE_' + key) || key;
                    var value = equip.specials[l].value;
                    if (value > 0) {
                        if (typeof expanderStatsDict[name] == 'undefined') expanderStatsDict[name] = 0;
                        expanderStatsDict[name] += value;
                    }
        ...
        var row = null;
        var count = 0;
        for(var key in expanderStatsDict) {
            if ((count % 2) == 0) {
                if (row) employeePanel.find('.expander-stats-table').append(row);
                row = $('<tr></tr>');
            }
            row.append('<td class="expander-stats">&nbsp;&nbsp;&nbsp;' + key + '</span> <span class="stats-value">+' + expanderStatsDict[key] + '</span></td>');
            count++;
        }
        if (row) employeePanel.find('.expander-stats-table').append(row);
        if (count == 0) {
            employeePanel.find('.expander-stats-table').hide();
        } else {
            employeePanel.find('.expander-stats-table').show();
        }
    ...
    function loadEquipment(panel, part) {
        ...
        var employee = player.getCardBySeqId(employeeId);
        for(var i=0;i<employee.equipments.length;i++) {
            var equip = employee.equipments[i];
            var equipID = equip.equipment_seq_id;
            ...
            panel.find('.item-expander-tab-container').empty();

            var shouldShowAll = equip.used_slot_count > 1;
            if (shouldShowAll) {
                var allSpan = $('<span>All</span>');
                allSpan.attr('id', 'item-expander-option-' + equipID + '-all');
                allSpan
                    .addClass('item-expander-option')
                    .addClass('item-expander-option-used')
                    .addClass('item-expander-option-all')
                    .addClass('item-expander-option-selected')
                    .addClass('item-expander-option-' + equipID);

                panel.find('.item-expander-tab-container').append(allSpan);
            }
            for(var j=0;j<equip.slot_count;j++) {
                var shouldSelect = (j==0 && !shouldShowAll);
                var expanderSpan = $('<span><img src="assets/expander_' + (j<equip.used_slot_count?'on':'off') + '.png" /></span>');
                expanderSpan.attr('id', 'item-expander-option-' + equipID + '-' + j);
                expanderSpan
                    .addClass('item-expander-option')
                    .addClass('item-expander-option-' + equipID);
                if (j < equip.used_slot_count) {
                    expanderSpan.addClass('item-expander-option-used');
                    if (shouldSelect) {
                        expanderSpan.addClass('item-expander-option-selected');
                    }
                }
                panel.find('.item-expander-tab-container').append(expanderSpan);
            }
            $('.item-expander-option-' + equipID).click(function() {
                var id = $(this).attr('id');
                var eID = id.split('-')[3];
                var gID = id.split('-')[4];

                if (!$(this).hasClass('item-expander-option-used')) return;

                $('.item-expander-option-' + equipID).removeClass('item-expander-option-selected');
                $(this).addClass('item-expander-option-selected');
                $('.item-expander-table-' + eID).hide();
                $('#item-expander-table-' + eID + '-' + gID).show();
            });
            ...
            //capacity, base_capacity, used_cost

            panel.find('.item-expander-content-container').empty();

            if (shouldShowAll) {
                var statsDict = {};
                for(var j=0;j<equip.expanders.length;j++) {
                    var expander = equip.expanders[j];
                    for(var k=0;k<STATS_KEYS.length;k++) {
                        var key = STATS_KEYS[k];
                        var name = KTTranslationManager.getUITranslatedText(BONUS_NAME_MAPPING[key]) || key;
                        var value = parseInt(expander[key]);
                        if (value > 0) {
                            if (typeof statsDict[name] == 'undefined') statsDict[name] = 0;
                            statsDict[name] += value;
                        }
                    }
                    for(var k=0;equip.specials && k<equip.specials.length;k++) {
                        var key = equip.specials[k].param_type_name;    //display_status , param_type_name
                        var name = KTTranslationManager.getUITranslatedText('EMPLOYEES_RUNE_BUFF_TYPE_' + key) || key;
                        var value = equip.specials[k].value;
                        statsDict[name] = value;
                    }
                }
                var allTable = $('<table class="stats-table"></table>');
                allTable.attr('id', 'item-expander-table-' + equipID + '-all');
                allTable.addClass('item-expander-table-' + equipID);
                var row = null;
                var count = 0;
                for(var key in statsDict) {
                    if ((count % 2) == 0) {
                        if (row) allTable.append(row);
                        row = $('<tr></tr>');
                    }
                    row.append('<td class="expander-stats">' + key + '</span> <span class="stats-value">+' + statsDict[key] + '</span></td>');
                    count++;
                }
                if (row) allTable.append(row);
                panel.find('.item-expander-content-container').append(allTable);
            }

            for(var j=0;equip.expanders && j<equip.expanders.length;j++) {
                var expander = equip.expanders[j];
                var statsDict = {};
                for(var k=0;k<STATS_KEYS.length;k++) {
                    var key = STATS_KEYS[k];
                    var name = KTTranslationManager.getUITranslatedText(BONUS_NAME_MAPPING[key]) || key;
                    var value = parseInt(expander[key]);
                    if (value > 0) statsDict[name] = value;    
                }
                for(var k=0;expander.specials && k<expander.specials.length;k++) {
                    var key = expander.specials[k].param_type_name;
                    var name = KTTranslationManager.getUITranslatedText('EMPLOYEES_RUNE_BUFF_TYPE_' + key) || key;
                    var value = expander.specials[k].value;
                    statsDict[name] = value;
                }

                var table = $('<table class="stats-table"></table>');
                table.attr('id', 'item-expander-table-' + equipID + '-' + j);
                table.addClass('item-expander-table-' + equipID);
                if (shouldShowAll || j > 0) table.attr('hidden', 'hidden');

                table.append('<tr><td colspan="2"><span class="expander-stats">'+KTTranslationManager.getUITranslatedText('EMPLOYEES_RUNE_COST')+'</span> <span class="stats-value">' + expander.used_cost + '/' + expander.capacity + '</span></td></tr>')
                ...
                if (row) table.append(row);
                panel.find('.item-expander-content-container').append(table);
            }
    ...
    function loadBattleUnit(unit) {
        ...
        if (isLeader) {
            ...
            unitPanel.find('.battle-leader-thumbnail').show();
        } else {
            unitPanel.find('.battle-leader-thumbnail').hide();
        }
        if (promo > 0) {
            unitPanel.find('.battle-promo-thumbnail').attr('src', 'assets/promo' + promo + '.png');
            unitPanel.find('.battle-promo-thumbnail').show();
        } else {
            unitPanel.find('.battle-promo-thumbnail').hide();
        }
        if (curHP == 0) {
            unitPanel.find('.battle-ko').attr('src', 'assets/ko.png');
            unitPanel.find('.battle-ko').show();
        } else {
            unitPanel.find('.battle-ko').hide();
        }

        unitPanel.find('.battle-current-hp').width(unitPanel.find('.battle-hp-container').first().width()*curHP/maxHP);
        var hpPercentage = curHP/maxHP*100;
        hpPercentage = hpPercentage.toFixed(0);
        unitPanel.find('.battle-current-hp-text').text(curHP + ' (' + hpPercentage + '%)');
        ...
        if (element > 0) {
            ...
            unitPanel.find('.battle-element-thumbnail').show();
            //name = '<img src="assets/attribute' + element + '.png"> ' + name;
        } else {
            unitPanel.find('.battle-element-thumbnail').hide();
        }
    ...
    function handleNews(log, callback) {
        ...
        content = content.replace('promotion_4', '<b>Department Chief (部長)</b>');
        content = content.replace('promotion_5', '<b>Branch Manager (支社長)</b>');
        ...
        if (startPos > -1) {
            ...
            if (params[0] == 'equipment') {
                ...
                var iconId = params[1];
                var lang = window.KTTranslationManager.contentLang;
                var name = KTGameDataManager.getEmployeeName(iconId, lang) || '';
                ...
                if (eventName == 'track') {
                    ...
                    dungeonName = KTTranslationManager.getContentTranslatedText('DUNGEON_AREA_' + dungeonName);
                    
                    content = content.replace(originalText, '<b>Footprint</b> on <b>Floor ' + floorId + '</b> in <b>' + dungeonName + '</b> (Server: <b>' + server + '</b>)');
                    content = logId + '<b>' + playerNameText + '</b> ' + content + '!';
                    appendNewsLogToNews(log.id, content, '../../assets/track.png');
                }
                callback(); 
    ...
    window.KTUIManager = {
        ...
        updateResourceUI: function(setFood, setIron, setStone, setWood) {
            ...
            var displayedName = player.getName();
            if (displayedName.length>10) {
                displayedName = displayedName.substr(0, 10) + '...';
            }
            $('#player-name').text(displayedName);

            var food        = player.getResource('food');
            var maxFood     = player.getResource('max_food');
            var regenFood   = player.getRecover('food');
            if (typeof setFood != 'undefined') food = setFood;

            var iron        = player.getResource('iron');
            var maxIron     = player.getResource('max_iron');
            var regenIron   = player.getRecover('iron');
            if (typeof setIron != 'undefined') iron = setIron;

            var stone       = player.getResource('stone');
            var maxStone    = player.getResource('max_stone');
            var regenStone  = player.getRecover('stone');
            if (typeof setStone != 'undefined') stone = setStone;

            var wood        = player.getResource('wood');
            var maxWood     = player.getResource('max_wood');
            var regenWood   = player.getRecover('wood');
        ...
        updateEmployeeList: function(employeeList) {
            if (typeof employeeList == 'undefined') {
                ...
                employeeList = player.getCardList();
            }
            ...
            } else if ($('#sort-select').val() == 'tenure') {
        ...
        updateSkillForAll: function() {
            ...
            var employeeList = player.getCardList();
        ...
        updateScheduleList: function(scheduleList) {
            ...
            for(var i=0;i<scheduleList.length;i++) {
                ...
                if (window.KTConfigManager.config['ui_language'] == KTConst.language.JP) {
                    title = scheduleList[i].jp_title;
                
                } else if (window.KTConfigManager.config['ui_language'] == KTConst.language.ZH) {
                    title = scheduleList[i].zh_title;
                }
        ...
        layoutPlayerNews: function() {
            ...
            var dungeonReportHeight = $('#dungeon-report-content').height();
            var playerNewsContentHeight = $('#player-news-content').parent().height() - scheduleHeight - cautiousModeHeight - dungeonReportHeight - 10;
        ...
        silentLogin: function() {
            ...
            var kToken = player.getKToken();
            if (!kToken) return;

            var text = window.KTTranslationManager.getUITranslatedText('SETTINGS_LABEL_LOGIN_NOTICE').replace('{name}', player.getKUsername());
        ...
        updateDungeonDefcon: function() {
            ...
            if (!player.defcon) return;

            var defcon = player.defcon;
        ...
        startDungeonReport: function() {
            var reports = KTGameLogManager.getDungeonReportList();
            $('#dungeon-report-content').empty();
            
            var count = 0;
            for(var i=0;i<reports.length;i++) {
                var report = reports[i];

                var remainTimeValue = (new Date(report['expire_date'] + ' GMT+0900')).valueOf() - nowTime();
                if (remainTimeValue < 0) continue;
                if (count > 3) continue;
                count++;

                var remainTime = new KGTime(remainTimeValue);
                
                var title = '[#' + report.report_seq_id + '] ';

                if (report.messages.length == 5) {
                    var areaName = report.messages[1].text;
                    var translatedAreaName = KTTranslationManager.getContentTranslatedText('DUNGEON_AREA_' + areaName);
                    var areaNameColor = report.messages[1].color;

                    var bonusName = report.messages[3].text;
                    var translatedBonusName = KTTranslationManager.getContentTranslatedText('DUNGEON_BONUS_' + bonusName);
                    var bonusNameColor = report.messages[3].color;

                    var message = KTTranslationManager.getContentTranslatedText('DUNGEON_REPORT_BONUS_MESSAGE');
                    title += message.replace('{area}', translatedAreaName)
                                    .replace('{area_color}', areaNameColor)
                                    .replace('{bonus}', translatedBonusName)
                                    .replace('{bonus_color}', bonusNameColor);

                } else if (report.messages.length == 6) {
                    var companyName = report.messages[0].text;
                    var companyNameColor = report.messages[0].color;
                    
                    var areaName = report.messages[2].text;
                    var translatedAreaName = KTTranslationManager.getContentTranslatedText('DUNGEON_AREA_' + areaName);
                    var areaNameColor = report.messages[2].color;
                    
                    var bossName = report.messages[4].text;
                    var translatedBossName = KTTranslationManager.getContentTranslatedText('DUNGEON_BOSS_' + bossName);
                    var bossNameColor = report.messages[4].color;

                    var message = KTTranslationManager.getContentTranslatedText('DUNGEON_REPORT_DEFEAT_MESSAGE');
                    title += message.replace('{company}', companyName)
                                    .replace('{company_color}', areaNameColor)
                                    .replace('{area}', translatedAreaName)
                                    .replace('{area_color}', areaNameColor)
                                    .replace('{boss}', translatedBossName)
                                    .replace('{boss_color}', bossNameColor);

                } else {
                    for(var j=0;j<report.messages.length;j++) {
                        var message = report.messages[j];
                        if (typeof message.color != 'undefined') {
                            title += '<span style="color:' + message.color + '">' + message.text + '</span>';
                        } else {
                            title += message.text;
                        }
                    }
                ...
                var scheduleText = title + ' <span style="float:right">' + remainTime + '&nbsp;</span>'

                $('#dungeon-report-content').append(
                    '<div class="schedule-item dungeon-report-item">&nbsp;' + scheduleText + '</div>'
                );
            }
            this.layoutPlayerNews();
        },
