(function(){
    ...
    function updateEmployeePanel(employeePanel, employee, callback) {
        ...
        if (!name || KTUIManager.lang == 'jp') name = employee.card_name;
        ...
        var policyName = KTUIManager.getTranslatedText('EMPLOYEES_POLICY_NAME_' + employee.policy_type);
        var policyDesc = KTUIManager.getTranslatedText('EMPLOYEES_POLICY_DESC_' + employee.policy_type);
        var raidPolicyName = KTUIManager.getTranslatedText('EMPLOYEES_POLICY_NAME_' + employee.vs_boss_policy_type);
        var raidPolicyDesc = KTUIManager.getTranslatedText('EMPLOYEES_POLICY_DESC_' + employee.vs_boss_policy_type);
        ...
        if (nextExp > 0) {
            ...
            employeePanel.find('.employee-remain-exp-text').text('Next: ' + (nextExp-currentExp).toLocaleString('en-US') + ' EXP');
    ...
    function loadEquipment(panel, part) {
        ...
        for(var i=0;i<employee.equipments.length;i++) {
            ...
            for(var key in equip.effects) {
                var name = KTUIManager.getTranslatedText(BONUS_NAME_MAPPING[key]) || key;
    ...
    function loadBattleUnit(unit, featureDict) {
        ...
        if (isLeader) {
            var policyName = KTUIManager.getTranslatedText('EMPLOYEES_POLICY_NAME_' + strategy);
            var policyDesc = KTUIManager.getTranslatedText('EMPLOYEES_POLICY_DESC_' + strategy);
            var raidPolicyName = KTUIManager.getTranslatedText('EMPLOYEES_POLICY_NAME_' + raidStrategy);
            var raidPolicyDesc = KTUIManager.getTranslatedText('EMPLOYEES_POLICY_DESC_' + raidStrategy);
        ...
        if (translatedName && KTUIManager.lang != 'jp') name = translatedName;
    ...
    window.KTUIManager = {
        ...
        updateRawResources: function(employees) {
            ...
            const nextText = KTUIManager.getTranslatedText('RESOURCE_NEXT');
        ...
        getSkillDetails: function(skill) {
            ...
            for(var i=0;i<skill.effects.length;i++) {
                ...
                if (targetType == 1) {
                    text += this.getTranslatedText('EMPLOYEES_SKILL_ALLY_BUFF') + ', ';
                } else if (targetType == 2) {
                    text += this.getTranslatedText('EMPLOYEES_SKILL_SELF_BUFF') + ', ';
                }
                ...
                if (phase.effect_type == 2) {
                    text += ', <b>' + phase.effect_value + (phase.effect_count>1?'*' + phase.effect_count:'') + ' ' + this.getTranslatedText('EMPLOYEES_SKILL_DAMAGE') + '</b>';
                } else {
                ...
                if (phase.long_distance == 1 && phase.magic != 1 && classId != '3') text += ' , ' + this.getTranslatedText('EMPLOYEES_SKILL_LONG');
                if (phase.magic == 3 || (phase.magic == 1 && classId != '8' && phase.skill_type == 1)) text += ', ' + this.getTranslatedText('EMPLOYEES_SKILL_SURE_HIT');

                if (skill.priority == 0) {
                    text += ' , ' + this.getTranslatedText('EMPLOYEES_SKILL_PREEMPTIVE');
                } else if (skill.priority == 2) {
                    text += ' , ' + this.getTranslatedText('EMPLOYEES_SKILL_MOVE_LAST');
                }
                

                if (phase.recovery && phase.recovery.states && phase.recovery.states.death) {
                    text += ', ' + this.getTranslatedText('EMPLOYEES_SKILL_RESSURECT');
                }

                if (phase.recovery && phase.recovery_debuff) {
                    text += ', ' + this.getTranslatedText('EMPLOYEES_SKILL_CLEANSE');
                    ...
                    if (phase.recovery.states) {
                        if ((!phase.recovery.states.death && Object.keys(phase.recovery.states) > 0) 
                            ...
                            text += ', ' + this.getTranslatedText('EMPLOYEES_SKILL_AILMENT_CLEANSE');        
                        }
                    }
                } else if (phase.recovery_debuff) {
                    text += ', ' + this.getTranslatedText('EMPLOYEES_SKILL_DEBUFF_CLEANSE');
                }            
                ...
                if (phase.buffs) {
                    ...
                    for(var key in phase.buffs) {
                        ...
                        if (buff.params) {
                            ...
                            text += ' (' + paramsText + this.getTranslatedText('EMPLOYEES_SKILL_BUFF_PERIOD').replace('{period}', buff.period) + ')';

                        } else {
                            text += ' (+' + (buff.effect_value/(buff.buff_type == 'attack'?100:10)) + '%, ' + this.getTranslatedText('EMPLOYEES_SKILL_BUFF_PERIOD').replace('{period}', buff.period) + ')';
                        }
                    }
                }
            ...
            if (skill.feedback) {
                text += '[' + this.getTranslatedText('EMPLOYEES_SKILL_COMPENSATION') + ' ' + (skill.feedback.effect_value / 10) + '%]<br>';
            }
        ...
        appendToNews: function(content) {
            if (KTConfigManager.config['news_order'] == 'newest-top') {
                $('#news-content').prepend(content + '<br>');    
            } else {
                $('#news-content').append(content + '<br>');
            }
        },
        ...
        convertUiIntoLanguage: function(lang) {
            if (typeof this.langDict[lang] == 'undefined') return;
            for(var key in this.langDict[lang]) {
                var value = this.langDict[lang][key];
                $('.' + key).html(value);
            }
        },
        ...
        reloadContentIntoLanguage: function(lang) {
            ...
            this.updateEmployeeList(cardList);
            this.updateAllTeams();
        },

        getTranslatedText: function(textKey) {
            if (typeof this.langDict[this.lang] == 'undefined') return textKey;
            if (typeof this.langDict[this.lang][textKey] == 'undefined') {
            ...
            return this.langDict[this.lang][textKey];
        } 
    }
})();