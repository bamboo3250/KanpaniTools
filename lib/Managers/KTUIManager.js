(function(){
    ...
    function getTeamPolicy(deck) {
        for(var i=0;i<6;i++) {
            ...
            if (employee.leader == '1') {
                return {
                    'normal': POLICIES[employee.policy_type].name,
                    'raid': POLICIES[employee.vs_boss_policy_type].name
                };
            }
        }
        return '';
    ...
    function loadBattleUnit(unit, featureDict) {
        ...
        if (isLeader) {
            teamPanel.find('.battle-policy').text(POLICIES[strategy].name);
        ...
        fetchCharaThumbnail(imageDir, function(url) {
    ...
    window.KTUIManager = {
        ...
        updateUI: function(setFood, setIron, setStone, setWood) {
        ...
        updateDeck: function(index) {
            ...
            for(var i=0;i<6;i++) {
                ...
                $(teamPanelSelector + ' .team-normal-policy').text(teamPolicy.normal);
                $(teamPanelSelector + ' .team-raid-policy').text(teamPolicy.raid);
            }
        },
