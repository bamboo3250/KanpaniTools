(function(){
    ...
    function updateEmployeePanel(employeePanel, employee, callback) {
        ...
        for(var j=0;j<employee.equipments.length;j++) {
            ...
            if (equip.anti_elements) {
                for(var k=0;k<equip.anti_elements.length;k++) {
                    ...
                    var translatedAntiName = KTTranslationManager.getUITranslatedText('EMPLOYEES_SKILL_BUFF_TYPE_' + antiElement.anti_element_name);
                    if (translatedAntiName.includes('EMPLOYEES_SKILL_BUFF_TYPE_')) {
                        translatedAntiName = antiElement.anti_element_name;
                    }
                    var effectText = translatedAntiName + '+' + antiElement.effect_value + '%';
            ...
            if (equip.anti_features) {
                for(var k=0;k<equip.anti_features.length;k++) {
                    ...
                    var translatedAntiName = KTTranslationManager.getUITranslatedText('EMPLOYEES_SKILL_BUFF_TYPE_' + antiFeature.anti_feature_name);
                    if (translatedAntiName.includes('EMPLOYEES_SKILL_BUFF_TYPE_')) {
                        translatedAntiName = antiFeature.anti_feature_name;
                    }
                    var effectText = translatedAntiName + '+' + antiFeature.effect_value + '%';
        ...
        for (var debuffKey in employee.resists) {
            if (employee.resists[debuffKey] > 0) {
                var translatedBuffType = KTTranslationManager.getUITranslatedText('EMPLOYEES_SKILL_BUFF_TYPE_' + debuffKey.toUpperCase());
                if (translatedBuffType.includes('EMPLOYEES_SKILL_BUFF_TYPE_')) {
                    translatedBuffType = buff.buff_type;
                }
                employeePanel.find('.resist-table tbody').append('<tr><td>' + translatedBuffType + ' +' + (employee.resists[debuffKey]/10) + '%</td></tr>');
                resistCount++;
            }
        }
        ...
        if (employee.debuff_resists) {
            for (var debuffKey in employee.debuff_resists) {
                if (employee.debuff_resists[debuffKey] > 0) {
                    ...
                    employeePanel.find('.resist-table tbody').append('<tr><td>' + translatedBuffType + ' +' + (employee.debuff_resists[debuffKey]/10) + '%</td></tr>');
                    resistCount++;
                }
            }
        }
    ...
    window.KTUIManager = {
            ...
            for(var i=0;i<skill.effects.length;i++) {
                ...
                if (phase.buffs) {
                    ...
                    for(var key in phase.buffs) {
                        ...
                        var translatedBuffType = KTTranslationManager.getUITranslatedText('EMPLOYEES_SKILL_BUFF_TYPE_' + buff.buff_type.toUpperCase());
                        if (translatedBuffType.includes('EMPLOYEES_SKILL_BUFF_TYPE_')) {
                            translatedBuffType = buff.buff_type;
                        }
                ...
                if (phase.erosion) {
                    ...
                    for(var key in phase.erosion.states) {
                        var translatedBuffType = KTTranslationManager.getUITranslatedText('EMPLOYEES_SKILL_BUFF_TYPE_' + key.toUpperCase());
                        if (translatedBuffType.includes('EMPLOYEES_SKILL_BUFF_TYPE_')) {
                            translatedBuffType = buff.buff_type;
                        }
                        text += ' ' + translatedBuffType;
                    }
                }
                ...
                if (phase.properties) {
                    ...
                    for(var key in phase.properties) {
                        ...
                        var translatedBuffType = KTTranslationManager.getUITranslatedText('EMPLOYEES_SKILL_BUFF_TYPE_' + phase.properties[key].property.toUpperCase());
                        if (translatedBuffType.includes('EMPLOYEES_SKILL_BUFF_TYPE_')) {
                            translatedBuffType = buff.buff_type;
                        }
                        text += ' ' + translatedBuffType;
        ...
        silentLogin: function() {
            ...
            var kToken = KTPlayerManager.k_token[playerId];
            if (!kToken) return;
