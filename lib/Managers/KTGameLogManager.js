(function(){
    ...
    window.KTGameLogManager = {
    	...
    	updateLogs: function(logs) {
    		...
    		this.updateLogsRecursively(logs, 0);
    	},

    	updateLogsRecursively(logs, index) {
    		if (index >= logs.length) return;

    		var self = this;
    		var log = logs[index];
			if (typeof this.gameLogs[log.id] != 'undefined') {
				this.updateLogsRecursively(logs, index+1);
				return;
			}
			this.gameLogs[log.id] = log;
			handleLog(log, function() {
				self.updateLogsRecursively(logs, index+1);
			});
    	}
    }
})();